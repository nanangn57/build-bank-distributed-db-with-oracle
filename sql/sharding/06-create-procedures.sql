...
CREATE OR REPLACE PROCEDURE create_account(
    ...
    p_currency VARCHAR2 DEFAULT 'USD',  -- Ignored, always USD
...
BEGIN
    ...
    -- Currency is always USD (enforced by trigger and constraint)
    INSERT INTO accounts (user_id, account_number, account_type, balance, currency, region)
    VALUES (p_user_id, p_account_number, p_account_type, p_balance, 'USD', v_user_region)
...
-- Uses account_number (globally unique) instead of account_id
CREATE OR REPLACE PROCEDURE transfer_money(
    p_from_account_number VARCHAR2,
    p_to_account_number VARCHAR2,
    p_amount NUMBER,
...
BEGIN
    ...
    -- Try to update on local shard first, then try other shards via database links
    DECLARE
    ...
    BEGIN
        ...
        -- If not found locally, try other shards via database links
        IF v_rows_updated = 0 THEN
            -- Try shard1 (NA)
            UPDATE accounts@shard1_link 
            SET balance = balance + p_amount, 
                last_updated = SYSDATE 
            WHERE account_number = p_to_account_number
            AND status = 'ACTIVE';
        ...
        -- Check if destination account was updated (exists and is active)
        IF v_rows_updated = 0 THEN
            RAISE_APPLICATION_ERROR(-20003, 'Destination account not found or not active');
        END IF;
    END;
    
    -- Create transaction record (on source account shard, using account_number)
    -- account_number = from_account_number (transaction stored on source account shard)
    ...
    VALUES (
        p_from_account_number,  -- account_number for routing (source account)
        p_from_account_number,
        p_to_account_number,
        'TRANSFER',
        p_amount,
        'COMPLETED',
    ...
    DBMS_OUTPUT.PUT_LINE('From Account: ' || p_from_account_number || ', Amount: ' || p_amount);
    DBMS_OUTPUT.PUT_LINE('To Account: ' || p_to_account_number);
...
BEGIN
    ...
    -- Create transaction record using account_number
    -- account_number = to_account_number (transaction stored on destination account shard)
    INSERT INTO transactions (
        account_number,
        from_account_number,
        to_account_number,
    ...
    VALUES (
        p_to_account_number,  -- account_number for routing (destination account)
        NULL,
        p_to_account_number,
        'DEPOSIT',
        p_amount,
    ...
    DBMS_OUTPUT.PUT_LINE('Account: ' || p_to_account_number || ', Amount: ' || p_amount);
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Account: ' || p_from_account_number || ', Amount: ' || p_amount);
...
LEFT JOIN transactions t ON a.account_number = t.from_account_number OR a.account_number = t.to_account_number
GROUP BY a.account_number, a.user_id, u.username, u.full_name,
...
PROMPT   - Transactions: Co-located with source account (uses account_number, globally unique)
PROMPT ====================================
